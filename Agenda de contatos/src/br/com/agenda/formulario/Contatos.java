...
public class Contatos extends JFrame {
	...
	private void adicionar() {
		...
		try {
			...
			pst.setString(1, txt_id.getText());
			rs = pst.executeQuery();
			// se o id já estiver sendo utilizado mostra a mensagem caso contrario cadastre
	...
	// Editar contato
	private void editar() {
		String editar = "UPDATE tb_contatos set nome=?, fone=?, email=? WHERE id=?";
		try {
			String pesquisar = "select * from tb_contatos where id = ?";
			pst = conexao.prepareStatement(pesquisar);
			// substituir o argumento ? pelo conteudo da caixa de testo txt_id
			...
			if (rs.next()) {
				...
				if (editado > 0) {
					JOptionPane.showMessageDialog(null, txt_nome.getText() + " Editado com sucesso!");
					limpar();
				} else {
					JOptionPane.showMessageDialog(null, "Não foi possivel editar o contato");
				}
			} else {
	...
	private void excluir(){
		String excluir = "DELETE FROM tb_contatos WHERE id=?";
		try{
			...
			if(excluido > 0){
				...
				JOptionPane.showMessageDialog(null, "Não foi possivel excluir o contato");
			}
		}catch(Exception e){
			System.out.println(e);
		}
	...
	public Contatos() {
		setIconImage(Toolkit.getDefaultToolkit()
				.getImage(Contatos.class.getResource("/br/com/agenda/icones/if_contacts_1055082.png")));
		...
		btn_edit.setIcon(
				new ImageIcon(Contatos.class.getResource("/br/com/agenda/icones/if_user_close_edit_103748.png")));
		...
		btn_remove.setIcon(
				new ImageIcon(Contatos.class.getResource("/br/com/agenda/icones/if_user_close_remove_103763.png")));
